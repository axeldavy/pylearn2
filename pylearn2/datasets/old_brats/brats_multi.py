...
class BRATS(Dataset):
    ...
    def get_label(self, brain_name, z, x, y, index):
    ...
    def next(self):
        if (self.stochastic and self.count >= self.num_minibatches_train) or ((not (self.stochastic)) and self.count >= self.num_minibatches_test):
            raise StopIteration()
        X = numpy.zeros((12, 32, 32, 128), dtype=numpy.float32)
        y = [numpy.zeros((128, 5), dtype=numpy.float32) for i in xrange(9)]
        index = 0
        if self.stochastic:
        ...
        for i in xrange(5):
            for j in xrange(3):
                ...
                for d in data:
                    for vn in xrange(9):
                        y[vn][index, self.get_label(d[0], d[1], d[2], d[3], vn)] = 1.
                    X[..., index] = self.get_patch(d[0], d[1], d[2], d[3])
                    index += 1

        mini_batch = [X]
        mini_batch.extend(y)
        mini_batch = tuple(mini_batch)
        assert index == 128
        self.count += 1
        if self.stochastic:
