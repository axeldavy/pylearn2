...
class BRATS(Dataset):
    ...
    def get_patch(self, brain_name, z, x, y):
        ...
        im_patch = numpy.concatenate([im_patch, orth1, orth2], axis=2)
    ...
    def iterator(self, mode=None, batch_size=None, num_batches=None,
        ...
        if batch_size <> 256:
            raise ValueError("only a batch_size of 256 is supported")
        self.count = 0
        if self.distribution is None:
            ...
            assert self.num_examples_labels.sum() == 256
        if rng is None:
            self.current_index_testing = numpy.zeros((5,3), dtype=numpy.int)
    ...
    def next(self):
        if (self.stochastic and self.count >= self.num_minibatches_train) or ((not (self.stochastic)) and self.count >= self.num_minibatches_test):
            raise StopIteration()
        X = numpy.zeros((12, 32, 32, 256), dtype=numpy.float32)
        y = numpy.zeros((256, 5), dtype=numpy.float32)
        index = 0
        if self.stochastic:
    ...
    def num_examples(self):
        ...
        """
        if self.stochastic:
            return self.num_minibatches_train * 256
        else:
            return self.num_minibatches_test * 256
