...
class BRATS(Dataset):
    ...
    TODO
    """
    def __init__(self, path_brains, path_analysis, patch_shape, num_minibatches_train=None, num_minibatches_test=None, axes = ('b', 0, 1, 'c'), max_labels=None, distribution=None):
...
# if the patch_shape is odd, we look at the real center
    ...
    def next(self):
        if (self.stochastic and self.count >= self.num_minibatches_train) or ((not (self.stochastic)) and self.count >= self.num_minibatches_test):
            raise StopIteration()
        X = numpy.zeros((self.num_channels_input, self.patch_shape[0], self.patch_shape[1], 128), dtype=numpy.float32)
