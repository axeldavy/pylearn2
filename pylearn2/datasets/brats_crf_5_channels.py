...
class BRATS(Dataset):
    ...
    TODO
    """
    def __init__(self, path_brains, path_analysis, patch_shape, output_shape, num_minibatches_train=None, num_minibatches_test=None, axes = ('b', 0, 1, 'c'), max_labels=None, distribution=None):
...
# if the patch_shape is odd, we look at the real center
    ...
    def next(self):
        if (self.stochastic and self.count >= self.num_minibatches_train) or ((not (self.stochastic)) and self.count >= self.num_minibatches_test):
            raise StopIteration()
        X = numpy.zeros(((5,) + self.patch_shape + (128,)), dtype=numpy.float32)
        y = numpy.zeros(((128,) + self.output_shape), dtype=numpy.int)
        index = 0
        if self.stochastic:
        ...
        for i in xrange(5):
            for j in xrange(3):
                ...
                assert len(data) == num_examples
                for d in data:
                    X_patch, y_patch = self.get_patch(d[0], d[1], d[2], d[3])
                    y[index, ...] = y_patch
                    X[..., index] = X_patch
                    index += 1
